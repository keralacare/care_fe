...
const SearchInput = ({
  ...
  return (
    <TextFormField
      ...
      leading={
        props.leading || (
          <CareIcon icon="l-search-alt" className="text-gray-600 z-10" />
        )
      }
      trailing={
        props.trailing ||
        (!props.secondary && (
          <div className="absolute inset-y-0 right-0 hidden py-1.5 pr-1.5 md:flex">
            <kbd className="inline-flex items-center rounded border border-secondaryActive bg-primary px-2 font-sans text-sm font-medium text-gray-500 focus:opacity-0">
              {shortcutKeyIcon}
            </kbd>
          </div>
        ))
      ...
      trailingFocused={
        <div className="absolute inset-y-0 right-0 hidden gap-1 py-1.5 pr-1.5 md:flex">
          <kbd className="inline-flex items-center rounded border border-secondaryActive bg-primary px-2 font-sans text-sm font-medium text-gray-500">
            Esc
          </kbd>
        </div>
      }
