    ...
    <div>
      ...
      <div
        ...
        className={`mb-8 mt-5 flex flex-1 flex-col items-center justify-center rounded-lg border-[3px] border-dashed px-3 py-6 ${
          dragProps.dragOver ? "border-accent-500" : "border-gray-500"
      ...
      >
        <svg
          ...
          viewBox="0 0 24 24"
          aria-hidden="true"
          className={`h-12 w-12 ${dragProps.dragOver && "text-accent-500"} ${
            dragProps.fileDropError !== "" ? "text-red-500" : "text-primary-700"
          }`}
        >
        ...
        <p
          className={`text-sm ${dragProps.dragOver && "text-accent-500"} ${
        ...
        {selectedFile && (
          <div className="flex pt-2">
            ...
            <span
              className="cursor-pointer px-5 text-sm text-accent-500"
              onClick={() => {
                setSelectedFile(undefined);
                setFileData([]);
                setErrors([]);
        ...
        {selectedFile ? (
          <>
            <span
              className="focus:ring-blue mx-auto mt-4 max-w-xs cursor-pointer items-center rounded-md border border-accent-500 bg-primary px-3 py-2 text-sm font-medium leading-4 text-accent-700 transition duration-150 ease-in-out hover:text-accent-500 hover:shadow focus:border-accent-300 focus:outline-none active:bg-gray-50 active:text-accent-800"
          ...
          <a
            className="focus:ring-blue mx-auto mt-4 max-w-xs items-center rounded-md border border-accent-500 bg-primary px-3 py-2 text-sm font-medium leading-4 text-accent-700 transition duration-150 ease-in-out hover:text-accent-500 hover:shadow focus:border-accent-300 focus:outline-none active:bg-gray-50 active:text-accent-800"
            href={sampleLink}
            target="_blank"
            download
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <label
          className="flex cursor-pointer items-center justify-center gap-1 rounded-lg border border-accent-500 bg-primary px-4 py-2 text-sm font-medium text-accent-500 transition-all hover:border-accent-400 hover:text-accent-400"
