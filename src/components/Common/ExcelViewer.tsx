...
const ExcelViewer = ({
  ...
  return (
    <DialogModal
      fixedWidth={false}
      className={`z-10 h-full w-full max-w-5xl flex-col gap-4 rounded-lg bg-primary p-4 shadow-xl md:p-6 ${className}`}
      onClose={() => {
        handleClose();
      }}
      title={title}
          ...
          <div className="flex w-full justify-center overflow-scroll rounded-lg border border-gray-200">
            {fileData && fileData[0] ? (
              <div className="overflow-x-auto">
                <table className="w-full table-auto border-collapse rounded-lg border border-gray-300 bg-primary shadow-md">
                    ...
                    {fileData
                      ...
                      .map((row, rowIndex) => {
                        ...
                        return (
                          <tr
                            key={currentRowIndex}
                            className={` ${rowIndex % 2 === 0 ? "bg-gray-50" : "bg-primary"} `}
                          >
                            {showCheckbox && (
